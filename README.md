# Free Law Admissions Prompt Studio

This is the GitHub-ready version of the free law school admissions prompt generator.

## Files

- `index.html` — the full standalone site

## What It Does

- generates copy-ready prompts for law school application review
- includes school packs for Cleveland State, Capital, Dayton, and general JD strategy
- supports dossier review, red flag detection, overlap checks, essay revision, scholarship prep, and interview prep
- stores your selections locally in the browser

## Publish On GitHub Pages

1. Create a new GitHub repository.
2. Upload the contents of this folder to the repo root.
3. Enable GitHub Pages in the repository settings.
4. Set the source to the default branch root.
5. Open the published site URL from GitHub Pages.

## Notes

- This version is fully static.
- No API key is required.
- No paid backend is required.
